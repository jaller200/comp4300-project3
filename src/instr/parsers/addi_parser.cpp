...
std::vector<Instruction> AddiParser::parse(const std::string& line) const {
    ...
    //      add dest, src1, imm
    //
    std::regex add_rgx("^(addi)\\s+(\\$\\w+),\\s*(\\$\\w+),\\s*(-?\\b(0x[0-9a-fA-F]+|0[0-7]*|[1-9][0-9]*|0b[0-1]+)\\b)");
    std::smatch match;
    ...
    if (regDest == 0)
        throw SyntaxError("Invalid Syntax for ADDI: Cannot write to the $zero register", trimmedLine);
